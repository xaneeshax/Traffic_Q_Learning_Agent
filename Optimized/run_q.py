...
class Sim():
    ...
    def __init__(self, epsilon=0.5, iters=500000, alpha=0.01, gamma=0.9, visualizer=False, qs = {}):
        ...
        if 'y' in ryg_state:
            ...
            if len(self.actions_taken) > 2:
                ...
                if self.actions_taken[-2] != ryg_state or self.actions_taken[-3] != ryg_state:
                    return [ryg_state]
        #green_possibilities = ['GGGgrGrrrrrrGrrrr', 'rrrrGrrrrGGrrrrrr', 'rrrrrrrrrGGGrrrrr', 'rrrrrGGggrrrrGGgg']
        if len(self.actions_taken) > 150:
            for i in range(149):
                if self.actions_taken[-i] != ryg_state:
                    break
            if ryg_state == 'GGGgrGrrrrrrGrrrr':
                return ['yyyyryrrrrrryrrrr']
            if ryg_state == 'yyyyryrrrrrryrrrr':
                return ['rrrrGrrrrGGrrrrrr', 'rrrrrrrrrGGGrrrrr', 'rrrrrGGggrrrrGGgg']
            if ryg_state == 'rrrrGrrrrGGrrrrrr':
                return ['rrrryrrrryyrrrrrr']
            if ryg_state == 'rrrryrrrryyrrrrrr':
                return ['GGGgrGrrrrrrGrrrr', 'rrrrrGGggrrrrGGgg']
            if ryg_state == 'rrrrrrrrrGGGrrrrr':
                return ['rrrrrrrrryyyrrrrr']
            if ryg_state == 'rrrrrrrrryyyrrrrr':
                return ['GGGgrGrrrrrrGrrrr', 'rrrrrGGggrrrrGGgg']
            if ryg_state == 'rrrrrGGggrrrrGGgg':
                return ['rrrrryyyyrrrryyyy']
            if ryg_state == 'rrrrryyyyrrrryyyy':
                return ['rrrrrrrrrGGGrrrrr', 'rrrrGrrrrGGrrrrrr']
 ...
 # gets the action to take (policy) for a given state
    ...
    def run_sim_with_policy(self):
        while self.step < self.iters:
            ...
            action_to_take = allowed_actions[max(action_vals)]
            self.actions_taken.append(action_to_take)
            traci.trafficlight.setRedYellowGreenState(self.junction, action_to_take)
            traci.simulationStep()
